...
export class PostFxPipeline {
    ...
    constructor(core) {
        ...
        core.events.on('postFxPipeline', 'renderer/post-process', this._onPostProcess, { priority: -100 });
        // EventBus API ──────────────────────────────────────────────────────────
        core.events.on('postFxPipeline', 'renderer/shader:add', (params) => {
            this.addPass(new ShaderPass(params.pass), params.target);
        });
        core.events.on('postFxPipeline', 'renderer/shader:remove', (params) => {
            this.removePass(params.name, params.target);
        });
        core.events.on('postFxPipeline', 'renderer/shader:toggle', (params) => {
    ...
    addPass(pass, target) {
        ...
        if (list.some(p => p.name === pass.name)) {
            throw new Error(`[PostFxPipeline] Pass "${pass.name}" already exists on target "${key}".`);
        }
        list.push(pass);
     ...
     * @param target   Layer name, or omit / `'screen'` for the full stage.
     */
    togglePass(name, enabled, target) {
        const key = target ?? 'screen';
        const list = this._passes.get(key);
        if (!list)
            return;
    ...
    _resolveTarget(key) {
        ...
        if (key === 'screen') {
            ...
            const result = this._core.events.emitSync('renderer/layer', { name: key });
...
//# sourceMappingURL=PostFxPipeline.js.map