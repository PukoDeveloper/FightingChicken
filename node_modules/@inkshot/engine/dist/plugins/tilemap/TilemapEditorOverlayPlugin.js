 ...
 */
export class TilemapEditorOverlayPlugin {
    namespace = 'mapeditor-overlay';
    dependencies = ['tilemap', 'input', 'mapeditor'];
    editorMeta = {
    ...
    init(core) {
        ...
        core.events.on(this.namespace, 'tilemap/loaded', (p) => {
            ...
            if (this._gfx && this._worldLayer) {
                this._worldLayer.addChild(this._gfx);
            }
        });
        core.events.on(this.namespace, 'tilemap/unloaded', () => {
            ...
            if (this._gfx && this._worldLayer) {
                this._worldLayer.removeChild(this._gfx);
            }
            this._gfx?.clear();
        });
        // ── Editor open / close ────────────────────────────────────────────────
        ...
        // ── Pointer tracking ───────────────────────────────────────────────────
        core.events.on(this.namespace, 'input/pointer:move', (p) => {
        ...
        core.events.on(this.namespace, 'renderer/pre-render', (_p) => {
            this._draw();
        });
    }
    destroy(core) {
     ...
     */
    _canvasToTile(canvasX, canvasY) {
        if (!this._mapData)
            return null;
        let worldX = canvasX;
        let worldY = canvasY;
        const camResult = this._core?.events.emitSync('camera/state', {});
        const camOutput = camResult?.output;
        if (camOutput &&
...
//# sourceMappingURL=TilemapEditorOverlayPlugin.js.map