...
export class DataManager {
    ...
    editorMeta = {
        ...
        events: ['data/load', 'data/get', 'data/getAll', 'data/unload'],
        schemas: {
            data: {
                folder: 'data',
    ...
    init(core) {
        ...
        const { events } = core;
        // ── data/load ────────────────────────────────────────────────────────────
        events.on(this.namespace, 'data/load', async (params, output) => {
            ...
            try {
                ...
                if (params.entries !== undefined) {
                    ...
                    if (!response.ok) {
                        throw new Error(`[DataManager] Failed to fetch "${url}": ${response.status} ${response.statusText}`);
                    }
                    raw = (await response.json());
                }
                else {
                    throw new Error('[DataManager] data/load requires either "file" or "entries".');
                }
                // Merge into (or create) the collection map.
                const collection = this._store.get(params.collection) ?? new Map();
        ...
        // ── data/get ─────────────────────────────────────────────────────────────
        events.on(this.namespace, 'data/get', (params, output) => {
        ...
        // ── data/getAll ──────────────────────────────────────────────────────────
        events.on(this.namespace, 'data/getAll', (params, output) => {
            const collection = this._store.get(params.collection);
        ...
        // ── data/unload ──────────────────────────────────────────────────────────
        events.on(this.namespace, 'data/unload', (params, output) => {
