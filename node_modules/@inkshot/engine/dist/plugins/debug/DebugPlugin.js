...
function layerColour(layer) {
    if (layer & CollisionLayer.SENSOR)
        return LAYER_COLOUR[CollisionLayer.SENSOR];
    if (layer & CollisionLayer.HITBOX)
        return LAYER_COLOUR[CollisionLayer.HITBOX];
    if (layer & CollisionLayer.HURTBOX)
        return LAYER_COLOUR[CollisionLayer.HURTBOX];
...
export class DebugPlugin {
    ...
    init(core) {
        ...
        core.events.on(this.namespace, 'physics/body:add', (p) => {
        ...
        core.events.on(this.namespace, 'physics/body:remove', (p) => {
            this._colliders.delete(p.entityId);
        });
        // -- Entity events: track live entities ---------------------------------
        core.events.on(this.namespace, 'entity/created', (p) => {
            this._entities.set(p.entity.id, p.entity);
        });
        core.events.on(this.namespace, 'entity/destroyed', (p) => {
            this._entities.delete(p.entity.id);
        });
        // -- Tilemap events: track map data for grid overlay --------------------
        core.events.on(this.namespace, 'tilemap/loaded', (p) => {
        ...
        // -- Render loop ---------------------------------------------------------
        core.events.on(this.namespace, 'core/render', (p) => {
            this._onRender(p);
        });
        // -- Public debug events -------------------------------------------------
        core.events.on(this.namespace, 'debug/overlay:toggle', (p, output) => {
            this._visible = p.visible !== undefined ? p.visible : !this._visible;
            output.visible = this._visible;
            this._applyVisibility();
        });
        core.events.on(this.namespace, 'debug/overlay:visible', (_p, output) => {
            output.visible = this._visible;
        });
        core.events.on(this.namespace, 'debug/event-log:filter', (p) => {
            this._eventFilter = (p.filter ?? '').toLowerCase();
        });
        core.events.on(this.namespace, 'debug/event-log:get', (_p, output) => {
    ...
    _setupPixi(core) {
        try {
            ...
            const worldResult = core.events.emitSync('renderer/layer', { name: 'world' });
            ...
            const screenResult = core.events.emitSync('renderer/layer:create', { name: 'debug', zIndex: DEBUG_LAYER_Z });
    ...
    _applyVisibility() {
        if (this._worldGfx)
            this._worldGfx.visible = this._visible;
        if (this._screenContainer)
            this._screenContainer.visible = this._visible;
    }
    // ---------------------------------------------------------------------------
    // Private – render loop
    // ---------------------------------------------------------------------------
    _onRender(params) {
        ...
        if (this._frameTimes.length > this._fpsHistorySize)
    ...
    _drawColliderShape(g, shape, x, y, colour) {
    ...
    _updateEntityInspector() {
        const text = this._entityText;
        if (!text)
            return;
        const lines = [`Entities: ${this._entities.size}`];
        let shown = 0;
