...
export class ScriptManager {
    ...
    init(core) {
        ...
        const { events } = core;
        // ── script/define ─────────────────────────────────────────────────────
        events.on(this.namespace, 'script/define', (params) => {
            this._scripts.set(params.script.id, params.script);
        });
        // ── script/run ────────────────────────────────────────────────────────
        events.on(this.namespace, 'script/run', (params) => {
            ...
            if (existing && priority < existing.priority) {
                console.warn(`[ScriptManager] script/run: instance "${instanceId}" is already running ` +
        ...
        // ── script/stop ───────────────────────────────────────────────────────
        events.on(this.namespace, 'script/stop', (params) => {
        ...
        events.on(this.namespace, 'script/register-command', (params) => {
            this._commands.set(params.cmd, params.handler);
        });
        // ── script/state:get ──────────────────────────────────────────────────
        events.on(this.namespace, 'script/state:get', (_params, output) => {
            const instances = [];
    ...
    // ---------------------------------------------------------------------------
    async _execute(script, instanceId, vars, priority) {
        ...
        core.events.emitSync('script/started', {
            id: script.id,
            instanceId,
        });
        ...
        if (this._running.get(instanceId) === state) {
            ...
            core.events.emitSync('script/ended', {
                id: script.id,
                instanceId,
            });
        }
     ...
     */
    async _executeNodes(script, state) {
        const core = this._core;
        if (!core)
            return;
        let nextIndex = 0;
        while (nextIndex < script.nodes.length && !state.stopped) {
            ...
            if (!handler) {
                console.warn(`[ScriptManager] Unknown command "${node.cmd}" at index ${index} ` +
                    `in script "${script.id}" (instance: "${state.instanceId}").`);
                continue;
            }
            core.events.emitSync('script/step', {
                id: script.id,
                instanceId: state.instanceId,
                index,
                cmd: node.cmd,
            ...
            try {
                ...
                console.error(`[ScriptManager] Error in command "${node.cmd}" at index ${index} ` +
                    `in script "${script.id}" (instance: "${state.instanceId}"):`, err);
                core.events.emitSync('script/error', {
                    id: script.id,
                    instanceId: state.instanceId,
                    index,
                    cmd: node.cmd,
                ...
                for (const fn of state.stopResolvers)
                    fn();
            }
            ...
            if (this._running.get(state.instanceId) !== state)
                return;
        }
    }
    /**
    ...
    _stopInstance(instanceId) {
        ...
        for (const fn of state.stopResolvers)
            fn();
        this._core?.events.emitSync('script/ended', {
            id: state.script.id,
            instanceId: state.instanceId,
        });
    }
    ...
    _registerBuiltinCommands() {
        ...
        this._commands.set('jump', (ctx) => {
            ...
            if (idx === -1) {
                console.warn(`[ScriptManager] jump: label "${target}" not found ` +
        ...
        this._commands.set('if', (ctx) => {
            const varRef = ctx.node.var;
            const op = ctx.node.op ?? 'eq';
            const expected = ctx.node.value;
            const labelName = ctx.node.jump;
            if (!varRef || !labelName) {
                console.warn('[ScriptManager] if: requires "var", "value", and "jump" fields.');
                return;
            }
            ...
            switch (op) {
                ...
                case 'lte':
                    ...
                    break;
                default:
                    console.warn(`[ScriptManager] if: unknown op "${op}". Supported: eq, ne, gt, lt, gte, lte.`);
                    return;
            }
            if (condition) {
                ...
                if (idx !== -1) {
                    ...
                    console.warn(`[ScriptManager] if: label "${labelName}" not found ` +
                        `in script "${ctx.script.id}".`);
                }
            }
        });
        ...
        this._commands.set('set', (ctx) => {
            ...
            if (varRef.startsWith('$')) {
                ...
                if (dot !== -1) {
                    ctx.core.events.emitSync('store/set', {
                        ns: name.slice(0, dot),
        ...
        this._commands.set('say', (ctx) => {
            ...
            const { output: stateOut } = ctx.core.events.emitSync('game/state:get', {});
            ...
            ctx.core.events.emitSync('dialogue/show-text', {
                text: ctx.node.text != null
                    ? this._interpolateText(ctx.node.text, ctx)
                    : undefined,
                speaker: ctx.node.speaker,
                portrait: ctx.node.portrait,
                speed: ctx.node.speed,
                i18nKey: ctx.node.i18nKey,
                i18nArgs: ctx.node.i18nArgs,
                speakerI18nKey: ctx.node.speakerI18nKey,
            });
            return new Promise((resolve) => {
                let restored = false;
                const restore = () => {
                    ...
                    restored = true;
                    if (lockGameState)
                        ctx.core.events.emitSync('game/state:set', { state: 'playing' });
                };
        ...
        this._commands.set('choices', (ctx) => {
            const rawChoices = ctx.node.choices;
            ...
            ctx.core.events.emitSync('dialogue/show-choices', {
                prompt: ctx.node.prompt != null
                    ? this._interpolateText(ctx.node.prompt, ctx)
                    : undefined,
                choices,
            });
            return new Promise((resolve) => {
                ...
                const off = ctx.core.events.once(this.namespace, 'dialogue/choice:made', (params) => {
                    const varName = ctx.node.var;
                    if (varName)
                        ctx.vars[varName] = params.index;
                    restore();
                    resolve();
                });
                ctx.onStop(() => { off(); restore(); resolve(); });
        ...
        this._commands.set('wait-event', (ctx) => {
            ...
            const varName = ctx.node.var;
            const timeoutMs = ctx.node.timeout;
            const timeoutJump = ctx.node.timeoutJump;
            return new Promise((resolve) => {
                let settled = false;
                const settle = () => {
                    if (settled)
                ...
                const off = ctx.core.events.on(this.namespace, event, (payload) => {
                    if (settled)
                        return;
                    if (varName)
                        ctx.vars[varName] = payload;
                    settle();
                });
                    ...
                    timeoutHandle = setTimeout(() => {
                        ...
                        if (timeoutJump) {
                            ...
                            if (idx !== -1) {
                                ...
                                console.warn(`[ScriptManager] wait-event: timeout label "${timeoutJump}" not found ` +
                                    `in script "${ctx.script.id}".`);
                            }
                        }
                        settle();
        ...
        this._commands.set('call', async (ctx) => {
            ...
            const extraVars = ctx.node.vars;
            if (extraVars)
                Object.assign(ctx.vars, extraVars);
        ...
        this._commands.set('fork', (ctx) => {
            ...
            const instanceId = ctx.node.instanceId ?? id;
            const vars = { ...(ctx.node.vars ?? {}) };
            const priority = ctx.node.priority ?? 0;
            // Priority check for the target instance
            const existing = this._running.get(instanceId);
            if (existing && priority < existing.priority) {
                console.warn(`[ScriptManager] fork: instance "${instanceId}" is already running ` +
        ...
        this._commands.set('wait-instance', (ctx) => {
            const instanceId = ctx.node.instanceId;
            if (!instanceId) {
                console.warn('[ScriptManager] wait-instance: missing "instanceId" field.');
                return;
            }
            // Already done (or never started) — resolve immediately
            if (!this._running.has(instanceId))
                return;
            return new Promise((resolve) => {
                const off = ctx.core.events.on(this.namespace, 'script/ended', (p) => {
    ...
    _resolveVar(ref, ctx) {
        ...
        const dot = name.indexOf('.');
        if (dot !== -1) {
            const { output } = ctx.core.events.emitSync('store/get', { ns: name.slice(0, dot), key: name.slice(dot + 1) });
            return output.value;
        }
     ...
     */
    _interpolateText(text, ctx) {
        return text.replace(/\{\$(\w+(?:\.\w+)?)\}/g, (_, ref) => {
     ...
     */
    _findLabel(script, labelName) {
        return script.nodes.findIndex((n) => n.cmd === 'label' && n.name === labelName);
    }
}
