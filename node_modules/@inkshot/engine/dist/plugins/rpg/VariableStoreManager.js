 ...
 * ```
 */
export class VariableStoreManager {
    namespace = 'store';
    /**
    ...
    init(core) {
        const { events } = core;
        // ── store/set ─────────────────────────────────────────────────────────
        events.on(this.namespace, 'store/set', (params) => {
            this._getOrCreate(params.ns)[params.key] = params.value;
        });
        // ── store/get ─────────────────────────────────────────────────────────
        events.on(this.namespace, 'store/get', (params, output) => {
            output.value = this._store.get(params.ns)?.[params.key];
        });
        // ── store/patch ───────────────────────────────────────────────────────
        events.on(this.namespace, 'store/patch', (params) => {
            Object.assign(this._getOrCreate(params.ns), params.patch);
        });
        // ── store/get-namespace ───────────────────────────────────────────────
        events.on(this.namespace, 'store/get-namespace', (params, output) => {
            output.data = this._store.get(params.ns);
        });
        // ── store/clear-namespace ─────────────────────────────────────────────
        events.on(this.namespace, 'store/clear-namespace', (params) => {
            this._store.delete(params.ns);
        });
        // ── store/snapshot ────────────────────────────────────────────────────
        events.on(this.namespace, 'store/snapshot', (_params, output) => {
            output.snapshot = this._deepCloneStore();
        });
        // ── store/restore ─────────────────────────────────────────────────────
        events.on(this.namespace, 'store/restore', (params) => {
            this._store.clear();
            const clone = structuredClone(params.snapshot);
        ...
        events.on(this.namespace, 'save/slot:load', (_params, output) => {
            if (!output.loaded || !output.raw)
                return;
            const raw = output.raw.data['_varStore'];
            if (raw && typeof raw === 'object' && !Array.isArray(raw)) {
                events.emitSync('store/restore', {
                    snapshot: raw,
                });
            }
        }, { phase: 'after' });
    ...
    _registerScriptCommands(core) {
        ...
        core.events.emitSync('script/register-command', {
            ...
            handler: (ctx) => {
                ...
                const { output } = core.events.emitSync('store/get', { ns, key });
                if (varName) {
        ...
        core.events.emitSync('script/register-command', {
            ...
            handler: (ctx) => {
                const ns = ctx.node['ns'];
                const patch = ctx.node['patch'];
                if (!ns || !patch || typeof patch !== 'object' || Array.isArray(patch)) {
                    console.warn('[VariableStoreManager] store-patch: requires "ns" (string) and "patch" (object) fields.');
                    return;
                }
                core.events.emitSync('store/patch', { ns, patch });
            },
        });
    }
}
//# sourceMappingURL=VariableStoreManager.js.map