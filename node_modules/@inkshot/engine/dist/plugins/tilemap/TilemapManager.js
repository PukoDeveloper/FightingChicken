...
export class TilemapManager {
     ...
     */
    setTile(layerIndex, col, row, tileId, options = {}) {
        if (!this._data)
            return;
        const layer = this._data.layers[layerIndex];
        if (!layer)
     ...
     */
    setLayerFilter(layerIndex, filters) {
    ...
    _buildTilesets(defs) {
        ...
        for (const def of defs) {
            const baseTexture = Assets.get(def.textureKey);
            if (!baseTexture) {
                throw new Error(`[TilemapManager] Texture "${def.textureKey}" has not been loaded. ` +
            ...
            const tilesPerRow = def.columns ??
                Math.floor((texW - margin * 2 + spacing) / (tileWidth + spacing));
            const tilesPerCol = Math.floor((texH - margin * 2 + spacing) / (tileHeight + spacing));
            ...
            for (let localId = 0; localId < totalTiles; localId++) {
                ...
                textures.set(localId, new Texture({
    ...
    _buildChunk(layerIndex, chunkX, chunkY) {
        ...
        if (existing) {
            ...
            for (const [tileId, refs] of this._animatedSpriteRefs) {
                const nextRefs = refs.filter((r) => !existing.sprites.includes(r.sprite));
        ...
        const sprites = new Array(totalSlots).fill(null);
        let tileCount = 0;
        for (let localRow = 0; localRow < this._chunkSize; localRow++) {
            for (let localCol = 0; localCol < this._chunkSize; localCol++) {
                ...
                if (worldCol >= mapData.mapWidth || worldRow >= mapData.mapHeight)
                    continue;
                const tileId = layerDef.data[worldRow * mapData.mapWidth + worldCol] ?? EMPTY_TILE;
     ...
     */
    _setTileRaw(layerIndex, col, row, tileId) {
        if (!this._data)
            return;
     ...
     */
    _applyAutoTile(layerIndex, col, row, tileId) {
        if (!this._data)
            return;
        ...
        for (const cell of cellsToUpdate) {
            const existingId = layerDef.data[cell.row * this._data.mapWidth + cell.col] ?? EMPTY_TILE;
            // Only recalculate neighbours that belong to the same group
            if (!(cell.col === col && cell.row === row) && !memberSet.has(existingId)) {
                continue;
            }
            const bitmask = this._computeBitmask(cell.col, cell.row, group.mode, memberSet, layerDef.data);
            const resolvedId = group.tileMap[bitmask] ?? existingId;
            layerDef.data[cell.row * this._data.mapWidth + cell.col] = resolvedId;
            affectedChunkKeys.add(this._chunkKey(layerIndex, Math.floor(cell.col / this._chunkSize), Math.floor(cell.row / this._chunkSize)));
        }
     ...
     */
    _getNeighbourCoords(col, row, mode) {
        if (!this._data)
            return [];
        const offsets4 = [
            { dc: 0, dr: -1 }, // N
     ...
     */
    _computeBitmask(col, row, mode, memberSet, layerData) {
        if (!this._data)
            return 0;
        const { mapWidth, mapHeight } = this._data;
        const isMember = (dc, dr) => {
            const nc = col + dc;
            const nr = row + dr;
     ...
     * tile shape grid.
     */
    _syncCollision(layerDef, mapData) {
        if (!this._core)
            return;
        const rows = [];
        for (let r = 0; r < mapData.mapHeight; r++) {
            rows.push(layerDef.data.slice(r * mapData.mapWidth, (r + 1) * mapData.mapWidth));
        }
    ...
    _chunkKey(layerIndex, chunkX, chunkY) {
