...
import { LAYER_Z_INDEX } from './layers.js';
...
export class Renderer {
    ...
    constructor(core) {
        ...
        for (const name of Object.keys(LAYER_Z_INDEX)) {
        ...
        core.events.on('renderer', 'renderer/layer', (params, output) => {
            output.layer = this.getLayer(params.name);
        });
        // Expose layer creation via the event bus.
        core.events.on('renderer', 'renderer/layer:create', (params, output) => {
    ...
    createLayer(name, zIndex) {
        if (this._layers.has(name)) {
            throw new Error(`[Renderer] Layer "${name}" already exists. ` +
