...
const DIALOGUE: DialogueLine[] = [
  {
    ...
    text: '……不……這不可能……我統治星空的混沌之力……被一隻小雞瓦解了？！',
    side: 'right',
  },
  {
    speaker: '小雞',
  ...
  },
  {
    ...
    portraitFactory: createChaosDisplay,
    accentColor: 0xcc44ff,
    text: '……你贏了。老者被封印在星空核心的水晶塔中。但……就算你到了那裡……',
    side: 'right',
  },
  {
    speaker: '勇氣',
  ...
  },
  {
    ...
    portraitFactory: createChaosDisplay,
    accentColor: 0xcc44ff,
    text: '……星空核心的守衛……比我更古老、更強大。我只是它的前哨而已……哈……哈哈……',
    side: 'right',
  },
  {
    speaker: '小雞',
  ...
  },
  {
    ...
    text: '孫兒……你的心之光越來越強……不管前方有什麼……都相信自己……',
    side: 'left',
  },
  {
    speaker: '勇氣',
...
async function enter(core: Core): Promise<void> {
  ...
  // ── Tick: blink tap hint ────────────────────────────────────────────────────
  const unsubTick = core.events.on('story_ch4_end', 'core/tick', () => {
    if (!advanceLocked) {
  ...
  _cleanup = () => {
    ...
    core.events.removeNamespace('story_ch4_end');
    unsubTick();
...
export const StoryChapter4EndScene: SceneDescriptor = {
  key: 'story_ch4_end',
  enter,
  exit,
};
