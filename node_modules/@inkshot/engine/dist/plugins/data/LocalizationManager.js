...
export class LocalizationManager {
    ...
    init(core) {
        ...
        const { events } = core;
        // ── i18n/load ──────────────────────────────────────────────────────────
        events.on(this.namespace, 'i18n/load', async (params, output) => {
            try {
                let raw;
                if (params.data !== undefined) {
                    ...
                    if (!response.ok) {
                        throw new Error(`[LocalizationManager] Failed to fetch locale file "${url}": ${response.status} ${response.statusText}`);
                    }
                    raw = (await response.json());
                }
                else {
                    throw new Error('[LocalizationManager] i18n/load requires either "url" or "data".');
                }
                const flat = this._flatten(raw);
                const existing = this._catalogues.get(params.locale) ?? new Map();
        ...
        // ── i18n/set-locale ────────────────────────────────────────────────────
        events.on(this.namespace, 'i18n/set-locale', async (params, output) => {
            if (!this._catalogues.has(params.locale)) {
                throw new Error(`[LocalizationManager] Locale "${params.locale}" has not been loaded. ` +
            ...
            await events.emit('i18n/changed', {
                locale: params.locale,
                previous,
            });
        });
        // ── i18n/t ─────────────────────────────────────────────────────────────
        events.on(this.namespace, 'i18n/t', (params, output) => {
        ...
        events.on(this.namespace, 'i18n/lookup', (params, output) => {
            if (params.key !== 'i18n' || params.value === null)
                return;
            const catalogue = this._currentLocale !== null
                ? (this._catalogues.get(this._currentLocale) ?? null)
                : null;
        ...
        events.on(this.namespace, 'i18n/interpolate', (params, output) => {
            ...
            output.result = output.result.replace(/\{([^{}:]+)(?::([^{}]*))?\}/g, (match, key, value) => {
                const { output: lookupOutput } = events.emitSync('i18n/lookup', { key, value: value ?? null }, { result: null });
                return lookupOutput.result ?? match;
            });
        }, { phase: 'before', priority: 1000 });
        // ── i18n/get-locales ───────────────────────────────────────────────────
        events.on(this.namespace, 'i18n/get-locales', (_params, output) => {
     ...
     */
    _flatten(data, prefix = '', result = new Map()) {
    ...
    _applyVars(template, vars) {
        return template.replace(/\{\{([^}]+)\}\}/g, (_match, name) => {
            return name in vars ? vars[name] : _match;
        });
    }
}
