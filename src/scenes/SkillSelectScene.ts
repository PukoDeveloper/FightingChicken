...
import { skillState, endlessState, gameResult, voidState } from '../game/store';
...
async function enter(core: Core): Promise<void> {
  ...
  backBtn.on('pointerdown', async () => {
    sfxMenuClick();
    const dest = (endlessState.active || voidState.active)
      ? 'modeselect'
      : (gameResult.storyMode ? 'story_dynamic' : 'levelselect');
