...
const _noopLayer = { addChild: () => { }, removeChild: () => { } };
// ---------------------------------------------------------------------------
// ParticleManager
// ---------------------------------------------------------------------------
/**
...
export class ParticleManager {
    ...
    init(core) {
        ...
        const result = core.events.emitSync('renderer/layer', { name: 'fx' });
        this._fxLayer = result.output.layer ?? this._fxLayer;
        // Advance all emitters on every fixed tick.
        core.events.on(this.namespace, 'core/update', (params) => {
            this._update(params.dt);
        });
        // ── EventBus public API ──────────────────────────────────────────────────
        core.events.on(this.namespace, 'particle/emit', (params, output) => {
            output.id = this.emit(params.config, params.id);
        });
        core.events.on(this.namespace, 'particle/stop', (params) => {
            this.stop(params.id);
        });
        core.events.on(this.namespace, 'particle/clear', (params) => {
            this.clear(params.id);
        });
        core.events.on(this.namespace, 'particle/move', (params) => {
            this.move(params.id, params.x, params.y);
        });
        core.events.on(this.namespace, 'particle/pause', (params) => {
            this.pause(params.id);
        });
        core.events.on(this.namespace, 'particle/resume', (params) => {
            this.resume(params.id);
        });
        core.events.on(this.namespace, 'particle/count', (_params, output) => {
            output.emitterCount = this.emitterCount;
            output.particleCount = this.particleCount;
        });
        core.events.on(this.namespace, 'particle/update', (params) => {
    ...
    emit(config, id) {
        ...
        const pool = new ObjectPool(() => this._createDisplay(config), this._poolSize);
        const emitter = {
            id: emitterId,
            config,
            active: true,
    ...
    _update(dt) {
        ...
        for (const id of completed) {
            this._core?.events.emitSync('particle/complete', { id });
        }
    }
    /**
    ...
    _stepEmitter(emitter, dt) {
        ...
        if (emitter.active && !emitter.config.burst) {
            if (emitter.config.duration !== undefined &&
    ...
    _spawnParticle(emitter) {
        ...
        const startRotationRad = degToRad(randomVariance(cfg.startRotation ?? 0, cfg.rotationVariance ?? 0));
        const angularVelocityRad = degToRad(randomVariance(cfg.angularVelocity ?? 0, cfg.angularVelocityVariance ?? 0)) / 1000;
        // Per-particle forces — convert px/s² → px/ms².
        const gravity = randomVariance(cfg.gravity ?? 0, cfg.gravityVariance ?? 0) / 1_000_000;
        const wind = randomVariance(cfg.wind ?? 0, cfg.windVariance ?? 0) / 1_000_000;
...
// ---------------------------------------------------------------------------
function defaultCreateDisplay(config) {
    if (config.texture) {
