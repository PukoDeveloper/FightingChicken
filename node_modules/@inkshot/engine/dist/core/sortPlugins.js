 ...
 */
export function sortPluginsByDependency(plugins) {
    ...
    for (let i = 0; i < plugins.length; i++) {
        ...
        if (byNamespace.has(p.namespace)) {
            throw new Error(`[createEngine] Duplicate plugin namespace: "${p.namespace}".`);
        }
    ...
    const dependents = new Map(); // dep ns → list of ns that need it
    ...
    for (const p of plugins) {
        for (const dep of (p.dependencies ?? [])) {
            if (!byNamespace.has(dep)) {
                throw new Error(`[createEngine] Plugin "${p.namespace}" declares a dependency on ` +
    ...
    if (result.length !== plugins.length) {
        const unresolved = plugins
            ...
            .map(p => p.namespace)
            .join(', ');
        throw new Error(`[createEngine] Circular plugin dependency detected among: ${unresolved}`);
    }
    return result;
}
