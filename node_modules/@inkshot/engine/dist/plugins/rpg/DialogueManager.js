import { parseDialogueMarkup, buildTextSegments, getSpeedAtIndex, } from './DialogueMarkupParser.js';
// ---------------------------------------------------------------------------
// DialogueManager
// ---------------------------------------------------------------------------
/**
...
export class DialogueManager {
    ...
    init(core) {
        this._core = core;
        const { events } = core;
        events.on(this.namespace, 'dialogue/show-text', (params) => {
            if (!this._session) {
                this._openSession();
            }
            ...
            events.emitSync('dialogue/node', { speaker, portrait: params.portrait });
        });
        events.on(this.namespace, 'dialogue/show-choices', (params) => {
            if (!this._session) {
                this._openSession();
            }
            const s = this._session;
            ...
            events.emitSync('dialogue/node', { speaker: '', portrait: undefined });
            events.emitSync('dialogue/choices', {
                choices: s.choices,
            });
        });
        events.on(this.namespace, 'dialogue/advance', () => {
            if (!this._session)
                return;
            if (!this._session.textDone) {
                ...
                events.emitSync('dialogue/text:tick', {
                    text: s.targetText,
                    done: true,
                    segments,
                });
            ...
            events.emitSync('dialogue/advanced', {});
        });
        events.on(this.namespace, 'dialogue/choice', (params) => {
            if (!this._session)
                return;
            ...
            events.emitSync('dialogue/choice:made', { index: params.index });
        });
        events.on(this.namespace, 'dialogue/end', () => {
            if (this._session) {
                this._endSession();
            }
        });
        events.on(this.namespace, 'dialogue/state:get', (_params, output) => {
            if (!this._session) {
                output.active = false;
                output.choices = [];
                output.text = '';
        ...
        events.on(this.namespace, 'core/update', (params) => {
            const s = this._session;
            if (!s || s.textDone)
                return;
            ...
            while (remainingMs > 0 && !s.textDone) {
                ...
                while (s.nextPauseIdx < s.parsed.pauses.length &&
            ...
            if (s.charIndex === prevCharIndex && !s.textDone)
                return;
            const segments = buildTextSegments(s.targetText, s.charIndex, s.parsed.colorSpans);
            events.emitSync('dialogue/text:tick', {
     ...
     */
    _resolveText(i18nKey, i18nArgs, raw, fallback) {
        if (i18nKey) {
            const { output } = this._core.events.emitSync('i18n/t', { key: i18nKey, args: i18nArgs });
    ...
    _openSession() {
        ...
        this._core.events.emitSync('dialogue/started', {});
    }
    ...
    _endSession() {
        if (!this._session)
            return;
        this._session = null;
        this._core.events.emitSync('dialogue/ended', {});
    }
}
