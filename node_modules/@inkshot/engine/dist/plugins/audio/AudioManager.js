...
export class AudioManager {
    ...
    init(core) {
        ...
        const { events } = core;
        // ── audio/load ────────────────────────────────────────────────────────────
        events.on(this.namespace, 'audio/load', async (params, output) => {
        ...
        // ── audio/play ────────────────────────────────────────────────────────────
        events.on(this.namespace, 'audio/play', (params, output) => {
            const buffer = this._buffers.get(params.key);
            if (!buffer) {
                throw new Error(`[AudioManager] No buffer loaded for key "${params.key}". ` +
        ...
        // ── audio/stop ────────────────────────────────────────────────────────────
        events.on(this.namespace, 'audio/stop', (params) => {
            if (params.instanceId !== undefined) {
                ...
                const toStop = [];
                for (const [id, inst] of this._instances) {
                    if (inst.category === params.category)
                        toStop.push(id);
                }
                for (const id of toStop)
                    this._stopInstance(id);
        ...
        // ── audio/pause ───────────────────────────────────────────────────────────
        events.on(this.namespace, 'audio/pause', (params) => {
        ...
        // ── audio/resume ──────────────────────────────────────────────────────────
        events.on(this.namespace, 'audio/resume', (params) => {
            const inst = this._instances.get(params.instanceId);
            if (!inst || inst.state !== 'paused')
                return;
            const buffer = this._buffers.get(inst.key);
            if (!buffer)
                return; // Buffer was unloaded while paused — cannot resume.
            const ctx = this._ctx;
        ...
        // ── audio/volume ──────────────────────────────────────────────────────────
        events.on(this.namespace, 'audio/volume', (params) => {
            const ctx = this._ctx;
            // Resolve the target gain in priority order:
            //   instanceId > category > master
            const gain = params.instanceId !== undefined
        ...
        // ── audio/fade-stop ───────────────────────────────────────────────────────
        events.on(this.namespace, 'audio/fade-stop', (params) => {
            const inst = this._instances.get(params.instanceId);
            if (!inst || inst.state !== 'playing')
                return;
            const ctx = this._ctx;
        ...
        // ── audio/unload ──────────────────────────────────────────────────────────
        events.on(this.namespace, 'audio/unload', (params, output) => {
            output.unloaded = this._buffers.delete(params.key);
        });
        // ── audio/state (pull) ────────────────────────────────────────────────────
        events.on(this.namespace, 'audio/state', (params, output) => {
            const inst = this._instances.get(params.instanceId);
            if (!inst) {
        ...
        // ── audio/list (pull) ─────────────────────────────────────────────────────
        events.on(this.namespace, 'audio/list', (_params, output) => {
            const instances = [];
            for (const [instanceId, inst] of this._instances) {
                if (inst.state === 'stopped')
                    continue;
                const currentTime = inst.state === 'playing' && this._ctx
        ...
        events.on(this.namespace, 'audio/listener:update', (params) => {
            const ctx = this._ctx;
            if (!ctx)
                return;
            ...
            if (typeof listener.positionX !== 'undefined') {
                ...
                // Legacy API
                listener.setPosition(params.x, -params.y, 1);
            }
        ...
        // ── audio/source:move ─────────────────────────────────────────────────────
        events.on(this.namespace, 'audio/source:move', (params) => {
            const inst = this._instances.get(params.instanceId);
            if (!inst)
                return;
            if (!inst.pannerNode) {
                ...
                // `audio/play`.
                console.warn(`[AudioManager] audio/source:move: instance "${params.instanceId}" ` +
