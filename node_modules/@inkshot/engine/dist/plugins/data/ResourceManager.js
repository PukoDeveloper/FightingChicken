...
export class ResourceManager {
    ...
    init(core) {
        ...
        const { events } = core;
        // ── assets/preload ────────────────────────────────────────────────────────
        events.on(this.namespace, 'assets/preload', async (params, output) => {
            let loaded = 0;
        ...
        // ── assets/load ───────────────────────────────────────────────────────────
        events.on(this.namespace, 'assets/load', async (params, output) => {
            try {
                if (params.definition !== undefined) {
                    ...
                    events.emitSync('assets/progress', {
                        progress: 1,
                        bundle: undefined,
                    });
                    output.loaded = true;
                    ...
                    throw new Error('[ResourceManager] assets/load requires one of: url, bundle, or definition.');
                }
            }
            catch (err) {
                output.loaded = false;
                const source = params.bundle ?? params.url ?? params.definition?.name ?? 'unknown';
                // Emit error asynchronously so subscribers get the full context.
                await events.emit('assets/error', {
                    message: String(err),
                    source,
                    error: err,
                });
        ...
        // ── assets/prefetch ───────────────────────────────────────────────────────
        events.on(this.namespace, 'assets/prefetch', (params) => {
        ...
        // ── assets/get ────────────────────────────────────────────────────────────
        events.on(this.namespace, 'assets/get', (params, output) => {
        ...
        // ── assets/unload ─────────────────────────────────────────────────────────
        events.on(this.namespace, 'assets/unload', async (params, output) => {
            try {
                if (params.bundle !== undefined) {
     ...
     */
    async _loadBundleByName(core, name) {
        await Assets.loadBundle(name, (progress) => {
            core.events.emitSync('assets/progress', {
                progress,
                bundle: name,
            });
        });
     ...
     */
    _assertBundleRegistered(bundle) {
        if (!this._registeredBundles.has(bundle)) {
            throw new Error(`[ResourceManager] Bundle "${bundle}" is not registered. ` +
