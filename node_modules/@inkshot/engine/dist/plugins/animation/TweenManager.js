...
export class Tween {
    ...
    seek(timeMs) {
        ...
        if (!this._started) {
            ...
            this._repeatDelayRemaining = 0;
            this._opts.onStart?.();
            const tgt0 = this.target;
            this._props = new Map(Object.entries(this._toProps).map(([key, to]) => [
                key,
                { from: tgt0[key] ?? 0, to },
            ]));
        }
...
export class TweenManager {
    ...
    init(core) {
        ...
        // ── EventBus API ────────────────────────────────────────────────────
        core.events.on(this.namespace, 'tween/to', (params, output) => {
        ...
        core.events.on(this.namespace, 'tween/kill', (params) => {
            if (params.all) {
                this.killAll();
                return;
            }
     ...
     */
    killTarget(object) {
        const toKill = [...this._active].filter((item) => item instanceof Tween && item.target === object);
