...
export class TimerManager {
    ...
    init(core) {
        const { events } = core;
        // ── timer/once ────────────────────────────────────────────────────────
        events.on(this.namespace, 'timer/once', (params) => {
        ...
        // ── timer/interval ───────────────────────────────────────────────────
        events.on(this.namespace, 'timer/interval', (params) => {
            this._timers.set(params.id, {
                id: params.id,
                type: 'interval',
        ...
        // ── timer/cancel ──────────────────────────────────────────────────────
        events.on(this.namespace, 'timer/cancel', (params) => {
            const hadTimer = this._timers.delete(params.id);
            const hadCooldown = this._cooldowns.delete(params.id);
            if (hadTimer || hadCooldown) {
                events.emitSync('timer/cancelled', { id: params.id });
            }
        });
        // ── timer/cancel-all ──────────────────────────────────────────────────
        events.on(this.namespace, 'timer/cancel-all', (_params, output) => {
            let count = 0;
            for (const id of [...this._timers.keys(), ...this._cooldowns.keys()]) {
                ...
                if (hadTimer || hadCooldown) {
                    count += 1;
                    events.emitSync('timer/cancelled', { id });
                }
            }
            output.cancelledCount = count;
        });
        // ── timer/cooldown ────────────────────────────────────────────────────
        events.on(this.namespace, 'timer/cooldown', (params, output) => {
        ...
        events.on(this.namespace, 'core/tick', (params) => {
            if (this._paused)
                return;
            ...
            for (const entry of this._timers.values()) {
                ...
                if (entry.type === 'once') {
                    if (entry.remaining <= 0) {
                        ...
                        events.emitSync('timer/fired', { id: entry.id, count: entry.count });
                    }
                }
                else {
                    ...
                    while (entry.remaining <= 0 && burstCount < MAX_BURST_FIRES) {
                        burstCount += 1;
                        entry.count += 1;
                        const done = entry.maxCount !== undefined && entry.count >= entry.maxCount;
                        events.emitSync('timer/fired', { id: entry.id, count: entry.count });
                        if (done) {
                            toRemove.push(entry.id);
                            break;
