...
export class SaveManager {
    ...
    init(core) {
        const { events } = core;
        // ── Slot events ───────────────────────────────────────────────────────────
        events.on(this.namespace, 'save/slot:set', (params) => {
            this._setSlot(params);
        });
        events.on(this.namespace, 'save/slot:get', (params, output) => {
            output.slot = this._slots.get(params.id);
        });
        events.on(this.namespace, 'save/slot:list', (_params, output) => {
            output.slots = Array.from(this._slots.values()).map((s) => ({ ...s.meta }));
        });
        events.on(this.namespace, 'save/slot:save', (params, output) => {
        ...
        events.on(this.namespace, 'save/slot:delete', (params, output) => {
            output.deleted = this._slots.delete(params.id);
        });
        // ── Global events ─────────────────────────────────────────────────────────
        events.on(this.namespace, 'save/global:set', (params) => {
            Object.assign(this._global.data, params.patch);
            this._global.updatedAt = Date.now();
        });
        events.on(this.namespace, 'save/global:get', (_params, output) => {
