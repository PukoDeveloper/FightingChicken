...
const DIALOGUE: DialogueLine[] = [
  {
    ...
    text: '……封印協議……失效。威脅等級……超出計算範圍。無法……無法繼續……',
    side: 'right',
  },
  {
    speaker: '小雞',
  ...
  },
  {
    ...
    portraitFactory: createGrandpaTurkeyDisplay,
    accentColor: 0xc8a060,
    text: '老骨頭還硬著呢，哈哈。但孩子——你能走到這裡，已經遠遠超越了我年輕時的自己。',
    side: 'left',
  },
  {
    speaker: '勇氣',
  ...
  },
  {
    ...
    portraitFactory: createGrandpaTurkeyDisplay,
    accentColor: 0xc8a060,
    text: '等一下。孩子，我在這裡發現了一個秘密……這片星空的黑暗，有一個更深的根源。',
    side: 'left',
  },
  {
    speaker: '小雞',
    portraitFactory: createChickenDisplay,
    accentColor: 0x66aaff,
    text: '我知道。但現在最重要的是把你帶回去。其他的事，我們一起面對！',
    side: 'left',
  },
  {
    speaker: '祖父',
    portraitFactory: createGrandpaTurkeyDisplay,
    accentColor: 0xc8a060,
    text: '……哈哈。好。我的孫兒長大了。走吧——但要記得，這場旅程還沒結束。',
    side: 'left',
  },
];
...
async function enter(core: Core): Promise<void> {
  ...
  // ── Tick: blink tap hint ────────────────────────────────────────────────────
  const unsubTick = core.events.on('story_ch5_end', 'core/tick', () => {
    if (!advanceLocked) {
  ...
  _cleanup = () => {
    ...
    core.events.removeNamespace('story_ch5_end');
    unsubTick();
...
export const StoryChapter5EndScene: SceneDescriptor = {
  key: 'story_ch5_end',
  enter,
  exit,
};
