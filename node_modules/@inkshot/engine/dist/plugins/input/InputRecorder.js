...
export class InputRecorder {
    ...
    init(core) {
        ...
        // ── Pull query ────────────────────────────────────────────────────────
        events.on(this.namespace, 'input/recorder:state', (_params, output) => {
        ...
        // ── Start recording ───────────────────────────────────────────────────
        events.on(this.namespace, 'input/recorder:start', () => {
            this._startRecording(core);
        });
        // ── Stop recording / playback ─────────────────────────────────────────
        events.on(this.namespace, 'input/recorder:stop', (_params, output) => {
            output.recording = this._stop(core);
        });
        // ── Playback ──────────────────────────────────────────────────────────
        events.on(this.namespace, 'input/recorder:play', (params) => {
            this._startPlayback(params.recording, params.loop ?? false);
        });
        // ── Pause / resume ────────────────────────────────────────────────────
        events.on(this.namespace, 'input/recorder:pause', () => {
            if (this._state === 'playing')
                this._state = 'paused';
        });
        events.on(this.namespace, 'input/recorder:resume', () => {
            if (this._state === 'paused')
                this._state = 'playing';
        });
        // ── Persistence ───────────────────────────────────────────────────────
        events.on(this.namespace, 'input/recorder:save', (params) => {
            this._saveRecording(core, params);
        });
        events.on(this.namespace, 'input/recorder:load', (params, output) => {
    ...
    _startRecording(core) {
        ...
        for (const eventName of RECORDED_EVENTS) {
            const unsubscribe = core.events.on(this.namespace, eventName, (params) => {
    ...
    _stop(core) {
        if (this._state === 'recording') {
            ...
            for (const unsub of this._recordingUnsubscribers)
                unsub();
        ...
        if (this._state === 'playing' || this._state === 'paused') {
            ...
            this._state = 'idle';
            if (rec) {
                core.events.emitSync('input/recorder:playback:end', { recording: rec });
            }
            return null;
        }
        return null;
     ...
     */
    _startPlayback(recording, loop) {
        if (this._core)
            this._stop(this._core);
     ...
     */
    _loadRecording(core, slotId) {
        const key = `${SAVE_KEY_PREFIX}${slotId}`;
        try {
            const { output } = core.events.emitSync('save/global:get', { key });
            if (typeof output.value !== 'string')
                return null;
            const parsed = JSON.parse(output.value);
            if (!this._isValidRecording(parsed))
                return null;
            return parsed;
        }
        catch {
...
//# sourceMappingURL=InputRecorder.js.map