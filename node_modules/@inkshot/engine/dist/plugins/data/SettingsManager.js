...
export class SettingsManager {
    ...
    init(core) {
        ...
        // ── settings/get-all ─────────────────────────────────────────────────────
        events.on(this.namespace, 'settings/get-all', (_params, output) => {
            output.settings = { ...this._settings };
        });
        // ── settings/reset ───────────────────────────────────────────────────────
        events.on(this.namespace, 'settings/reset', (params) => {
        ...
        // ── settings/save ─────────────────────────────────────────────────────────
        events.on(this.namespace, 'settings/save', async (_params, output) => {
            ...
            const { output: saveOut } = await events.emit('save/global:save', {});
            output.saved = saveOut.saved ?? false;
        });
        // ── settings/load ─────────────────────────────────────────────────────────
        events.on(this.namespace, 'settings/load', async (_params, output) => {
            // Ask the env plugin to populate the global save from storage.
            await events.emit('save/global:load', {});
            // Read the now-populated global data.
            const { output: globalOut } = events.emitSync('save/global:get', {});
     ...
     */
    _notifyChanged(changes) {
        this._core.events.emitSync('settings/changed', { changes });
    }
    /**
