 ...
 */
export function loadRpgData(gameData) {
    ...
    // ── Actors ────────────────────────────────────────────────────────────────
    const actors = (gameData.actors ?? []).map(_buildActorDef);
    // ── Items ─────────────────────────────────────────────────────────────────
    const items = (gameData.items ?? []).map((entry) => ({ ...entry }));
    ...
    // ── Status effects ────────────────────────────────────────────────────────
    const statusEffects = (gameData.statusEffects ?? []).map((e) => ({ ...e }));
    // ── Standalone scripts ────────────────────────────────────────────────────
    const scripts = (gameData.scripts ?? []).map((s) => ({ ...s }));
    return {
        gameTitle,
        version,
        locale,
...
function _buildStatProfile(cls) {
...
function _buildActorDef(entry) {
    const initialState = {
        ...(entry.initialState ?? {}),
    };
    if (entry.name !== undefined)
        initialState['name'] = entry.name;
    if (entry.classId !== undefined)
        initialState['classId'] = entry.classId;
    initialState['level'] = entry.initialLevel ?? 1;
    const scripts = (entry.scripts ?? []).map((s) => ({ ...s }));
    const triggers = (entry.triggers ?? []).map((t) => ({
        id: t.id,
        event: t.event,
        script: t.script,
        mode: t.mode ?? 'blocking',
        priority: t.priority ?? 10,
        onEnd: t.onEnd ?? 'nothing',
    }));
    return {
        id: entry.id,
        scripts,
